import numpy as np
import pandas as pd
#%matplotlib inline sklearn.decomposition
...
sns.heatmap(data =
cust.corr(), annot = True, fmt = ".2f", cmap = "cividis_r")
font = {"family":"Sherif",
"size":16}
plt.subplots_adjust(left = 1, bottom = 1,right = 2.5, top = 2, wspace = 0.5, hspace = None)
plt.subplot(1,2,1)
plt.pie(x = [len(cust[cust.Gender == 1]) , len(cust[cust.Gender == 0])] , labels = ['Male' , 'Female'],
...
cluster_dict.items()]
plt.legend(title='color', handles=handles, bbox_to_anchor=(1.05, 1), loc='uppe r left')
